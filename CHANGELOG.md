...
- **Move PC Pokémon between boxes** — select PC Pokémon in the All table and move them in bulk to a chosen destination box. New `POST /pc/move` endpoint with backend/local parity; rejects known-unsafe fallback slots (currently box 23 slot 4, whose byte range overlaps its section's footer).
